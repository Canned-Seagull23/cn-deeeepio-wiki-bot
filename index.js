...
function imgExistsPromise(name) {
    return new Promise((resolve, reject) => {
        client.getImageInfo(name, (err) => {
            if (err)
                reject(false);
            else
                resolve(true);
...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (let i in Object.keys(animals)) {
        ...
        const d = yield getArticlePromise(animalName)
            .catch(err => {
            console.error(err);
        ...
        if (tags.filter(tag => tag.tag == CDWB_TAGS.skintable.all.start).length != 0) {
            ...
            for (let i in undocumentedSkins) {
                ...
                yield imgExistsPromise(skin.id + ".png").then(() => __awaiter(void 0, void 0, void 0, function* () {
        ...
        yield editArticlePromise(animalName, lines.join('\n'), editSummary, false);
    }
    /*
